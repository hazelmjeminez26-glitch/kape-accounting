...
// Suggest the right small unit based on the inventory item's storage unit.
function suggestUnit(inventoryUnit: string): AmountUnit {
...
// Convert from recipe unit to inventory unit (used for deduction in Stage 3).
export function toInventoryAmount(amount: number, amountUnit: string, inventoryUnit: string): number {
  const au = amountUnit.toLowerCase();
  const iu = inventoryUnit.toLowerCase();
  if (au === "g"  && iu === "kg") return amount / 1000;
  if (au === "ml" && (iu === "liters" || iu === "litres" || iu === "l" || iu === "liter")) return amount / 1000;
  return amount;
}

// Human-readable conversion hint shown in the recipe editor.
function conversionHint(amount: number, amountUnit: string, inventoryUnit: string): string {
  const converted = toInventoryAmount(amount, amountUnit, inventoryUnit);
  if (converted === amount) return "";
  const displayUnit = (inventoryUnit === "liters" || inventoryUnit === "litres") ? "L" : inventoryUnit;
  const str = Number.isInteger(converted) ? String(converted) : converted.toFixed(4).replace(/\.?0+$/, "");
